...
\paragraph{Empirical result.} On the PlantCLEF 2026 leaderboard, the phenology prior at $\beta{=}1.0$ produced a Macro-F1 of \textbf{0.41346} versus the unaugmented DINOv3 baseline of 0.41826 ($\Delta = -0.0048$). The detailed sweep and discussion are reported in \S\ref{sec:experiments}.
...
Our anchor configuration is an DINOv3 last-blocks tile-inference ensemble: 4$\times$4 tiling, softmax-mean aggregation per tile, logit adjustment with $\tau{=}0.25$, and probability-threshold selection at $T{=}0.03$ (mean $k\!\approx\!2.92$). This configuration scores \textbf{0.41826} Macro-F1 on the PlantCLEF 2026 leaderboard.
...
\midrule
blk vs.\ blk\_8 & 0.901 & --- \\
D3@224 vs.\ D3@336 & 0.807 & 0.664 \\
4-way ens.\ vs.\ base & --- & 0.923 \\
\bottomrule
\end{tabular}
\end{table}

The 0.923 Jaccard between the 4-way intra-DINOv3 ensemble and our DINOv3 submission demonstrates all within-DINOv3 variants are statistically equivalent, motivating the pivot to fundamentally different evidence sources.

\subsection{Pivot 1: Triple-Backbone Classifier Retraining (cRT)}

We trained two MLP classifier heads on a frozen 3328-d concatenation of \mbox{BioCLIP-2 ViT-B}, DINOv3 ViT-L, and ConvNeXt-V2-L features (head A: standard CE; head B: class-balanced random sampling). Best validation accuracy: 63.95\%. We applied the trained heads at inference using the same 4$\times$4 tile / softmax-mean aggregation as DINOv3.

\textbf{Diagnostic gate.} Top-1 agreement between cRT and the DINOv3 baseline was only 0.094 (vs.\ 0.807 for two DINOv3 resolutions of itself), confirming structural orthogonality. Submission Jaccard at $T{=}0.03$ was 0.078.

\textbf{Submission.} Mixing the head A / head B mean of cRT with the DINOv3 224+336 ensemble at $w{=}0.3$ scored \textbf{0.38195} Macro-F1, $\Delta = -0.036$.

\textbf{Reading.} cRT is structurally orthogonal but \emph{empirically wrong} on the test distribution: a peakier posterior (mean $k\!=\!1.33$ standalone) means high-confidence wrong picks dominate the ensemble. The 9\% top-1 agreement was diversity going \emph{against} signal rather than complementing it.

\subsection{Pivot 2: Genus / Family Co-occurrence Reranking}

For each quadrat, we computed aggregated genus and family supports
\begin{align}
P_{\text{gen}}(g) &= \!\!\sum_{s\in g}\! p_{\text{DINOv3}}(s), \\
P_{\text{fam}}(f) &= \!\!\sum_{s\in f}\! p_{\text{DINOv3}}(s),
\end{align}
and applied a "siblings stick together" log-prior:
\begin{multline}
\log p_{\text{new}}(s) = \log p_{\text{DINOv3}}(s) + \alpha_g \log P_{\text{gen}}(g(s)) \\ + \alpha_f \log P_{\text{fam}}(f(s)).
\end{multline}

We swept $\alpha_g \in \{0,0.1,0.2,0.3,0.5\}$, $\alpha_f \in \{0,0.05,0.1,0.2\}$, $T \in \{0.025,0.03,0.035\}$ and submitted the \emph{isovolumetric} candidate $\alpha_g{=}0.1$, $\alpha_f{=}0.0$, $T{=}0.035$ (mean $k{=}2.89$, matched to the DINOv3 baseline volume; submission Jaccard with the DINOv3 baseline $=$ 0.887). Score: \textbf{0.41246}, $\Delta = -0.006$.
...
The full methodology is described in \S\ref{sec:phenology}. We swept $\beta \in \{0.25, 0.5, 1.0, 1.5, 2.0\}$ at three probability thresholds. The natural Bayesian operating point ($\beta{=}1.0$, no temperature scaling, $T{=}0.03$ to match the DINOv3 baseline) flips 12.8\% of top-1 picks at mean $k{=}3.24$. Score: \textbf{0.41346}, $\Delta = -0.005$.
...
\textbf{Reading.} Three concurrent factors plausibly explain the negative result: \textit{(i)} GBIF observation effort is itself summer-biased, diluting the discriminative season signal; \textit{(ii)} the DINOv3 backbone trained on dated images already encodes seasonal phenotype implicitly via leaf / flower / fruit state; \textit{(iii)} 11\% of species fall back to a uniform pdf precisely on the long-tail where the visual model is most uncertain --- so phenology cannot break ties where it would be most useful.
...
\begin{tabular}{lcc}
\toprule
Config & Macro-F1 & $\Delta$ vs.\ D3 \\
\midrule
D3 224+336, $T{=}0.03$ & \textbf{0.41826} & --- \\
\hspace{1em}+ cRT triple & 0.38195 & $-0.036$ \\
\hspace{1em}+ Genus $\alpha_g{=}0.1$ & 0.41246 & $-0.006$ \\
\hspace{1em}+ Pheno., $\beta{=}1.0$ & 0.41346 & $-0.005$ \\
\bottomrule
\end{tabular}
\caption{All four submitted configurations on the PlantCLEF 2026 hidden test set. The $\Delta$ shrinks monotonically as the priors become more information-theoretically orthogonal to the visual posterior --- a signature of DINOv3 implicitly absorbing all three signals through training-distribution co-occurrence.}
\label{tab:summary}
\end{table}
