...
At inference time each quadrat is resized to a $1792 \times 1792$ pixel canvas, partitioned into a four by four grid of non-overlapping $448$ pixel tiles, and each tile is then downsampled to the encoder's native $224$ pixel input. The fine-tuned encoder is applied to each tile and the per tile species logits are mapped to probabilities via softmax. The sixteen tile distributions are aggregated by per species mean. This aggregator was selected after an ablation against per species max, mean of top three, and noisy or; mean was the most numerically stable on the long tail and least sensitive to occasional confidently wrong tiles, dominating max aggregation by roughly 0.04 Macro F1 on the i002 last-four-blocks checkpoint.
We replace the standard top $k$ selection with an adaptive thresholded rule. After aggregation we emit every species whose mean probability exceeds $T$, subject to a floor of $k_{\min}{=}2$ and a ceiling of $k_{\max}{=}10$. A sweep of $T \in \{0.02, 0.03, 0.05\}$ together with several gap and relative thresholds was performed on each checkpoint; the best operating point for the i002 last four blocks checkpoint at the encoder's native $224$ pixel resolution is $T{=}0.05$. Adaptive selection contributes between 0.005 and 0.008 Macro F1 over the corresponding fixed top three submission on the same checkpoint.
...
\paragraph{Empirical result.} On the PlantCLEF 2026 leaderboard, the phenology prior at $\beta{=}1.0$ produced a Macro-F1 of \textbf{0.41346} versus the unaugmented BioCLIP 2.5 anchor of 0.418265 ($\Delta = +0.0018$). The detailed sweep and discussion are reported in \S\ref{sec:experiments}.
...
\textbf{Submission.} Mixing the head A / head B mean of cRT with the BioCLIP 2.5 anchor at $w{=}0.3$ scored \textbf{0.38195} Macro F1, $\Delta = -0.030$ versus the $0.418265$ anchor.
...
\midrule
BC2.5 last 4 blocks, $T{=}0.05$ & \textbf{0.418265} & --- \\
...
The configuration that produced our best public score is the BioCLIP 2.5 last-four-blocks anchor from experiment i002, submitted as a single model rather than an ensemble. The encoder is BioCLIP 2.5 ViT-H/14 with the last four transformer blocks plus the final layer norm and projection unfrozen and fine-tuned on the enlarged i001 manifest (2.65 million single-plant images across 7,806 species; see \S\ref{sec:cap_extra}) for ten epochs with the joint species/genus/family cross-entropy loss of \S\ref{sec:bioclip25_010}. At inference, each quadrat is resized to $1792 \times 1792$ pixels, partitioned into a $4 \times 4$ grid of non-overlapping $448$-pixel tiles, each tile is downsampled to the encoder's native $224$-pixel input, per-tile softmax probabilities are aggregated by species-wise mean across the sixteen tiles, and we emit every species whose mean probability exceeds $T{=}0.05$ subject to $k_{\min}{=}2$ and $k_{\max}{=}10$. Of the four configurations we submitted, three (the anchor, the genus/family rerank, and the phenology prior) land within $0.002$ Macro F1 of one another and a fourth (the triple-backbone cRT mix) falls $0.030$ below; we accordingly take the unaugmented anchor as our headline submission.
\section{Conclusion}
