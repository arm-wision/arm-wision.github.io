...
%%
%% Document class options (per the canonical ceurart.cls):
%%   (default)         single-column proceedings layout
%%   [twocolumn]       two-column proceedings layout
%%   [hf]              show running header / footer
%%   [longmktitle]     extra room for long author / affiliation lists
%%   [singleblind]     blind-review (single)
%%   [doubleblind]     blind-review (double)
%%   [final|review]    final-paper or review-mode layout
%%
...
\label{sec:pivot3}
The full methodology is described in \S\ref{sec:phenology}. We swept $\beta \in \{0.25, 0.5, 1.0, 1.5, 2.0\}$ at three probability thresholds. The natural Bayesian operating point ($\beta{=}1.0$, no temperature scaling, $T{=}0.03$, $\tau{=}0.25$ to match the BioCLIP 2.5 anchor) flips $12.8\%$ of top one picks at mean $k{=}3.24$. Score: \textbf{0.41346}, $\Delta = -0.005$.
...
\end{acknowledgments}
%%
%% Bibliography. The canonical ceurart.cls already issues
%% \bibliographystyle{elsarticle-num-names}; no explicit override needed.
