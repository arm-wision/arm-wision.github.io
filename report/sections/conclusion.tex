...
four transformer blocks unfrozen, trained on the enlarged PlantCLEF 2024 manifest with
taxonomy-aware auxiliary heads and deployed under tiled softmax-mean inference with class-prior
logit adjustment and a $224{+}336$ resolution ensemble. It reached a public Macro F1 of
\textbf{0.418265} and a private Macro F1 of \textbf{0.40283}.

Three findings carry the bulk of what this benchmark taught us. The first is that partial unfreeze
is a sharp, not a plateau, design choice on BioCLIP 2.5: $n{=}4$ unfrozen blocks is the empirical
sweet spot, deviations of one block in either direction cost between $0.009$ and $0.014$ Macro F1,
and a full fine-tune destroys the Tree-of-Life prior and drops the model to $0.208$. The second is
...
change---and we point to \S\ref{sec:discussion} for the limitations and the directions for future
