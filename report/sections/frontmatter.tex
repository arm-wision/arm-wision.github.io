...
plants and test images show dense multi-species plots which is further compounded by a
long-tailed label distribution. Our system is BioCLIP~2.5 ViT-H/14 where
only the last four transformer blocks plus the final layer norm and
projection are unfrozen, while the lower twenty-eight blocks stay
frozen to preserve the Tree-of-Life prior. We train on the
PlantCLEF~2024 single-plant manifest with auxiliary genus- and
family-level cross-entropy heads that regularise the species head
without adding inference cost. At inference each quadrat is split into a $4{\times}4$ grid of
...
the emitted species set. Our best public Macro F1 is \textbf{0.41826} and our best private
Macro F1 is \textbf{0.40283}.
\end{abstract}
