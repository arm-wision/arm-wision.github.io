...
final model is the second stage of a two-stage schedule: ten epochs of head-only training on the
enlarged manifest with the backbone fully frozen, which initialises the per-head MLPs of
\S\ref{sec:bioclip25_010}, then a further ten epochs with the last four blocks unfrozen, resuming
the model weights only. Full optimiser and hardware settings are listed in
Appendix~\ref{app:experiment_summary}.
