...
statement of the deployed configuration (\S\ref{sec:final_submission}). Throughout this
...
(experiment~i001) re-downloaded the corpus, deduplicated it, and pre-filled genus and family
labels for every row, yielding roughly $2.65$\,million images across $7{,}806$ species. The
deployed model (experiment~i002) trains on this larger manifest \emph{without} any per-species
cap. We also explored an explicitly rebalanced variant (experiment~i003) that augments the tail
with extra validated images and caps every species at five hundred images; its leaderboard
outcome, which did not improve on the uncapped manifest, is analysed in
Section~\ref{sec:longtail}. We therefore treat
``more clean data, no cap'' as a deliberate design choice for the final system rather than as a
controlled study of rebalancing.
...
Training runs in bfloat16 across two RTX~5090 GPUs with a low backbone learning rate
($1{\times}10^{-6}$) and a higher head learning rate ($1{\times}10^{-4}$), one epoch of linear
warmup followed by cosine decay, weight decay $1{\times}10^{-4}$, and label smoothing $0.1$. The
final model trains for ten epochs on the enlarged manifest; full optimiser and hardware settings
are listed in Appendix~\ref{app:experiment_summary}.
...
linear projection to a hidden representation, a GELU non-linearity, and dropout), but the two
fine-tunes we report wire it differently.
...
\textbf{Per-head MLPs (experiment~i002).} The deployed model instead gives each head its own
...
fine-tune and $L = \{g,f\}$ for the deployed per-head model, and the auxiliary weights are
...
the encoder. The deployed per-head model uses only $w_g{=}0.30$ and $w_f{=}0.15$. Missing taxonomy
...
embedding. We instead resize each quadrat to a $1792\times1792$ canvas and partition it into a
$4\times4$ grid of sixteen non-overlapping $448$-pixel tiles, so that most tiles contain only one
or a few species, closer to the training distribution. Each tile is forwarded through the encoder
...
dominate while evidence still accumulates across tiles. The final system uses softmax-mean. Plain
mean was explored on an earlier head-only checkpoint (experiment~010) and dropped; it was not
re-tested on the final model, so the two-stage comparison behind the softmax-mean choice is
reported, with that caveat, in Section~\ref{sec:tile_aggregation}.
...
The deployed system is a single BioCLIP~2.5 ViT-H/14 backbone with its last four transformer
blocks (plus the final layer norm and projection) fine-tuned for ten epochs on the enlarged i001
manifest under the joint species/genus/family cross-entropy loss of \S\ref{sec:bioclip25_010}. At
inference (Figure~\ref{fig:pipeline}), each quadrat is resized to $1792\times1792$ pixels and
partitioned into a $4\times4$ grid of non-overlapping $448$-pixel tiles; every tile is forwarded
...
$T{=}0.03$ is emitted, subject to $k_{\min}{=}2$ and $k_{\max}{=}10$. The two-resolution average is
a single-checkpoint ensemble of the deployed i002 model, not a multi-checkpoint ensemble; it is
produced by a separate inference and submission harness wrapped around that checkpoint rather than
inside the i002 training folder. This configuration is our
headline submission, scoring $0.418265$ public and $0.40283$ private Macro~F1 on the PlantCLEF~2026
