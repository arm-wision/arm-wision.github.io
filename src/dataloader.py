...
class PlantDALIPipeline(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, file_paths, labels, training=True):
        ...
        self.cpoint_flip = ops.Flip(
            device="gpu", vertical=0, 
            horizontal=ops.random.CoinFlip(probability=0.5) if training else 0
        )
...
def get_dali_loaders(csv_path, img_dir, batch_size=128, val_split=0.1, num_threads=4, device_id=0):
    ...
    train_loader = DALIGenericIterator(
            [train_pipe], ['data', 'label'], size=len(train_paths), auto_reset=True
    )
    val_loader = DALIGenericIterator(
            [val_pipe], ['data', 'label'], size=len(val_paths), auto_reset=True
    )
