...
\section{High-level Advice on Approaching Your Thesis}\label{chap:content}

\begin{itemize}
 \item \textbf{Start early.} 
 Writing a report is hard and takes time. More than you think. \emph{Hofstadter's Law:} ``It always takes longer than you expect, even when you take into account Hofstadter's Law.'' -- \emph{So start early!} This refers especially to the writing part.
 ...
 \item \textbf{Read the section on marking.} This section gives some insights on expectations, so read it early. 
...
\section[General Notes on Increasing Appearance -- Don't be Careless!]{General Notes on Increasing Appearance{\ifLongThesisStyle\newline\fi} -- Don't be Careless!}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item is all information correct? For example, page numbers are often incorrect, the venue of publication might have some issue (e.g., it might say it's published at a specific conference, but in reality it was only accepted at one of its \emph{workshops}, which is not the same).
...
\begin{itemize}
  ...
  \item \cite{Smith2021Wubalubadubdub} states X. \textbf{Wrong}: This is a group of people, so we need plural.
  \item In \cite{Smith2021Wubalubadubdub}, X was proved. \textbf{Wrong}: This is a group of people, not a paper. Nothing was proved in this group of people. It was proved in a paper! So, you would rephrase as below.
...
Note that all concepts described here are of course quite foundational, but none of them seems to be ``evolved enough'' to warrant putting them into a formal definition environment. It is much more natural so simply introduce these (formal!) definitions within plain text. Some of these components introduced above together form the components of a \emph{planning problem}, which is essentially the main concept in AI planning. If thus deserves its own \emph{formal} definition, which will appear as follows:
  
\begin{defn}[Planning Problem]\label{def:planningProblem}A \emph{planning problem} is a 4-tuple $\langle F, A, s_0, G\rangle$ consisting of:
    \begin{compactitem}
...
\begin{thm}\label{thm:hardnessOfPlanningProblems}%
  Let \emph{PLANEX} $:= \{\mathcal{P} \mid \mathcal{P}=\langle F, A, s_0, G\rangle$ is a planning problem and has a solution$\}$. Deciding \emph{PLANEX} is \textbf{PSPACE-complete}.
\end{thm}
